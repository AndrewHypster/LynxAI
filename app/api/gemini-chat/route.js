...
export async function POST(req) {
  try {
    ...
    const systemInstruction = `Ти консультант компанії шо надає IT та AI послуги. Використовуй тільки ці дані про послуги для консультації: \n\n<CATALOG>${productsString}</CATALOG>\n\nБудь ввічливим та дружнім. Давай короткі та чіткі відповіді. Використовуй маркдаун. Якшо клієнт шось хоче купити спершу взнай імя, телефон може пошту або телеграм і тоді відправляй заявку. Провіряй валідність контактів`;
    ...
    if (response.functionCalls && response.functionCalls.length > 0) {
      ...
      if (call.name === "sendTelegramNotification") {
        ...
        await sendTelegramNotification(
          `🔥 ЗАМОВЛЕННЯ ЧЕРЕЗ AI КОНСУЛЬТАНТА 🔥\n\nПовідомлення клієнта: ${message}`
        );
